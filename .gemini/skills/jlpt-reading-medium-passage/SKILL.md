...
> **Vòng lặp BẮT BUỘC: sửa → refresh CSV (nếu sửa HTML) → quay lại BƯỚC 2 (QC lại TẤT CẢ 30 mục TỪ ĐẦU, KHÔNG chỉ check mục đã FAIL) → nếu còn FAIL thì lặp lại.**
...
- [ ] Đã chạy QC checklist 30 mục TRỌN VẸN ở vòng cuối (không skip)
- [ ] **TẤT CẢ 30/30 mục đều PASS** (không có FAIL nào, không có "skip", không có "n/a")
...
Chỉ khi **TẤT CẢ 30 checks PASS + GATE 4→5 PASSED** → log:
```
